...
export function TweetReplyCard({ 
  ...
  return (
    <div 
      className="px-4 pb-1 pt-3 hover:bg-accent/50 transition-colors relative"
      onClick={handleContainerClick}
    >
      {/* Threading lines - connector from above */}
      {showConnector && (
        <div className="absolute left-9 top-0 w-0.5 bg-border h-6"></div>
      )}
      
      {/* Threading lines - continuation to below */}
      {!isLast && (
        <div className="absolute left-9 top-6 w-0.5 bg-border" style={{ height: 'calc(100% - 1.5rem)' }}></div>
      )}
